...
impl <World> CucumberRegistrar<World> for Matcher<World> {
...
mod test {
  ...
  fn matcher_add_step() {
    ...
    matcher.given(file!(), line!(), regex::build("^I do a basic thing$"), Box::new(move |_, _, _| InvokeResponse::Success));
  }

  #[test]
  fn matcher_find_match() {
    type World = u32;
    let mut matcher: Matcher<World> = Matcher::new();
    matcher.given("file", 0, regex::build("^I do (\\d+) basic things?$"), Box::new(move |_, _, _| InvokeResponse::Success));

    let mut matches = matcher.find_match("I do 6 basic things");
  ...
  fn macro_one_arg_works() {
    ...
    let mut world = 0;
    let mut matcher: Matcher<World> = Matcher::new();
    Given!(matcher; "^I do (\\d+) basic things?$", |_, _, (count,): (u32,)| {
      InvokeResponse::check_eq(count, 1)
    });
    assert_eq!(matcher.invoke("I do 1 basic thing", &mut world, None), InvokeResponse::Success);
  }
  ...
  fn macro_two_args_work() {
    ...
    let mut world = 0;
    let mut matcher: Matcher<World> = Matcher::new();
    Given!(matcher; "^I( don't)? do (\\d+) basic things?$", |_, _, (negate, count): (Option<String>, u32)| {
      if negate.is_some() {
    ...
    assert_eq!(matcher.invoke("I don't do 2 basic things", &mut world, None), InvokeResponse::Success);
  }
  ...
  fn macro_incorrect_arg_count() {
    ...
    let mut world = 0;
    let mut matcher: Matcher<World> = Matcher::new();
    Given!(matcher; "^I cant count my args$", |_, _, (_,): (u32,)| {
      InvokeResponse::Success
    });
    assert_eq!(matcher.invoke("I cant count my args", &mut world, None),
  ...
  fn macro_unparsable_arg() {
    ...
    let mut world = 0;
    let mut matcher: Matcher<World> = Matcher::new();
    Given!(matcher; "^I want a number but got a \"(.*)\"$", |_, _, (_,): (u32,)| {
      InvokeResponse::Success
    });
    assert_eq!(matcher.invoke("I want a number but got a \"word\"", &mut world, None),
